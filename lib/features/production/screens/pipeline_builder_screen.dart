...
class _ScrapItemsField extends StatelessWidget {
  const _ScrapItemsField({
    ...
    required this.onChanged,
    this.label = 'Scrap',
    this.addLabel = 'Add scrap item',
  });

  /// An assembly step routes rejected pieces, not scrap — same mechanism,
  /// different word for what came off the line.
  final String label;
    ...
    return Column(
      ...
      children: [
        ...
        _ScrapItemDropdown(
          ...
          items: items,
          units: units,
          label: scrapItems.isEmpty ? label : addLabel,
...
class _NodePropertiesPanelState extends State<_NodePropertiesPanel> {
  ...
  /// What to restore when the Assembly toggle is switched back off.
  String _nonAssemblyProcessType = 'Action';
  ...
  void initState() {
    ...
    _rememberNonAssemblyType(widget.node.processType);
    _attachListeners(widget.draft);
  }

  bool get _isAssemblyStep =>
      widget.draft.processType.text.trim().toLowerCase() == 'assembly';

  /// So toggling Assembly off restores whatever the step was before (e.g. a
  /// 'Cutting' step generated by Batch Add Flow) instead of flattening it to
  /// the generic 'Action'.
  void _rememberNonAssemblyType(String processType) {
    final trimmed = processType.trim();
    if (trimmed.isEmpty || trimmed.toLowerCase() == 'assembly') {
      return;
    }
    _nonAssemblyProcessType = trimmed;
  }

  void _setAssemblyStep(bool isAssembly) {
    if (isAssembly) {
      _rememberNonAssemblyType(widget.draft.processType.text);
    }
    setState(() {
      // Writing the controller fires the draft listener, which debounces into
      // provider.saveNodeDraft — the same path every other field uses.
      widget.draft.processType.text = isAssembly
          ? 'Assembly'
          : _nonAssemblyProcessType;
      // The step is just called Assembly. Only rename a stage still carrying a
      // generated name — a name the user typed is theirs to keep.
      final name = widget.draft.name.text.trim();
      if (isAssembly && _isGeneratedStageName(name)) {
        widget.draft.name.text = 'Assembly';
      } else if (!isAssembly && name == 'Assembly') {
        widget.draft.name.text = 'Stage ${widget.node.stageIndex + 1}';
      }
    });
  }

  static final RegExp _generatedStageName = RegExp(
    r'^(stage|step|process)\s*\d*$',
    caseSensitive: false,
  );

  bool _isGeneratedStageName(String name) =>
      name.isEmpty || _generatedStageName.hasMatch(name);

  /// The steps whose output flows into this one — what actually gets assembled
  /// here, in place of a bound input item.
  List<String> _feederStepNames() {
    final template = widget.provider.template;
    final feederIds = template.flows
        .where((f) => f.toNodeId == widget.node.id)
        .map((f) => f.fromNodeId)
        .toSet();
    return template.nodes
        .where((n) => feederIds.contains(n.id))
        .map((n) => n.name.trim().isEmpty ? n.processType : n.name)
        .toList(growable: false);
  }
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            // An assembly step is a merge point, not a machining stage: the
            // things being assembled arrive on the flows from whatever steps
            // feed into it, and the work is done by people. There is no single
            // input or output item to bind, and no machine or die to reserve —
            ...
            if (_isAssemblyStep) ...[
              ...
              // and the count lands on the worker's job card.
              if (FeatureFlags.isEnabled(FeatureKeys.pipelineMultiScrapItems))
                _ScrapItemsField(
                  ...
                  addLabel: 'Add rejection destination',
                  onChanged: (updated) {
            ...
            if (!_isAssemblyStep && !_isOutputNode) ...[
...
/// Stands in for the fields an assembly step doesn't have: it lists what feeds
/// into the step instead, since that is what determines what gets assembled.
class _AssemblySummary extends StatelessWidget {
  const _AssemblySummary({required this.feederNames});

  final List<String> feederNames;
      ...
      child: Column(
        ...
        children: [
          ...
          if (feederNames.isEmpty)
            Text(
              'Nothing feeds this step yet. Connect the steps whose output '
              'gets assembled here.',
              style: _panelHintStyle,
            )
          else
            Wrap(
              spacing: 7,
          ...
          const SizedBox(height: 12),
          Text(
            'No item, machine or die is set on an assembly step — the parts '
            'arrive on these flows and the work is done by hand. Reaching this '
            'step opens a job in Jobs for a worker to pick up.',
            style: _panelHintStyle,
          ),
        ],
      ),
...
const TextStyle _panelLabelStyle = TextStyle(
  fontFamily: 'Inter',
  fontSize: 13,
  fontWeight: FontWeight.w700,
  color: Color(0xFF334155),
...
const TextStyle _panelHintStyle = TextStyle(
  fontFamily: 'Inter',
  fontSize: 11.5,
  height: 1.4,
...
/// Marks a stage as assembly work. An Assembly node is what the backend
/// watches for when a run reaches it — it opens a pending freelancer job (plus
/// its BOM tasks), which is how the stage surfaces in Jobs for worker
/// assignment.
class _AssemblyStepToggle extends StatelessWidget {
  const _AssemblyStepToggle({required this.value, required this.onChanged});
  ...
  Widget build(BuildContext context) {
    return Material(
      color: value ? const Color(0xFFF1F0FE) : const Color(0xFFF8FAFC),
      borderRadius: BorderRadius.circular(14),
      child: InkWell(
        borderRadius: BorderRadius.circular(14),
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(14),
            border: Border.all(
              color: value ? const Color(0xFFC7C2F7) : const Color(0xFFD8E0EA),
            ),
          ),
          child: Row(
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const Text(
                      'Opens a job in the Jobs tab when production reaches '
                      'this stage, so it can be assigned to workers.',
                      style: TextStyle(
                        fontFamily: 'Inter',
                        fontSize: 11.5,
                        height: 1.35,
              ...
              Switch(
                value: value,
                onChanged: onChanged,
                activeThumbColor: const Color(0xFF6049E3),
              ),
            ],
          ),
        ),
