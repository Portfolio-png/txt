...
class ApiSearchRepository implements SearchRepository {
  ...
  Future<List<SearchResult>> search(String query) async {
    ...
    final uri = Uri.parse('$baseUrl/api/search?q=${Uri.encodeQueryComponent(query)}');
  ...
  Future<void> logSearchQuery(String query) async {
    final uri = Uri.parse('$baseUrl/api/search/history');
    await _client.post(uri, headers: const {'Content-Type': 'application/json'}, body: jsonEncode({'query': query}));
  }

  @override
  Future<void> logSearchClick(String query, SearchResult result) async {
    final uri = Uri.parse('$baseUrl/api/search/clicks');
    await _client.post(uri, headers: const {'Content-Type': 'application/json'}, body: jsonEncode({
      'query': query,
