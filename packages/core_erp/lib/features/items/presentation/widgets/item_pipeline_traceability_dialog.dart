...
class ItemPipelineTraceabilityDialog extends StatefulWidget {
  const ItemPipelineTraceabilityDialog({super.key, required this.item});
...
class _ItemPipelineTraceabilityDialogState
  ...
  Future<void> _fetchPipelineHistory() async {
    ...
    try {
      final uri = Uri.parse(
        '$_baseUrl/api/items/${widget.item.id}/pipeline-history',
      );
  ...
  void _generateSampleProvenance() {
    ...
    final pipelineName =
        widget.item.defaultPipelineName ?? 'Primary Manufacturing Pipeline';
    ...
    setState(() {
      _runs = [
        ...
        },
        {
          ...
          'status': 'completed',
          'startedAt': now
              .subtract(const Duration(days: 6, hours: 5))
              .toIso8601String(),
          'completedAt': now
  ...
  Widget build(BuildContext context) {
    ...
    final itemName = widget.item.name.isNotEmpty
        ? widget.item.name
        : 'Finished Item #${widget.item.id}';
    ...
    final avgYield = totalRawInflow > 0
        ? (totalGoodOutput / totalRawInflow) * 100
        : 74.0;
  ...
  Widget _buildRunCard(BuildContext context, Map<String, dynamic> run) {
    ...
    final pipelineName =
        run['pipelineName']?.toString() ?? 'Production Pipeline';
            ...
            Row(
              children: [
                ...
                _buildMetricPill(
                  label: 'Good Yield',
                  value:
                      '${goodOutputKg.toStringAsFixed(0)} kg (${yieldPct.toStringAsFixed(1)}%)',
                  color: const Color(0xFF2E7D32),
                ),
                const SizedBox(width: 8),
                _buildMetricPill(
