...
class VendorHistoryRepository {
  VendorHistoryRepository({required http.Client client, required this.baseUrl})
    : _client = client;

  final http.Client _client;
  final String baseUrl;

  Future<List<DeliveryChallanItem>> getVendorPurchaseHistory(
    int vendorId,
  ) async {
        ...
        return rawHistory.map((h) {
          return DeliveryChallanItem(
            ...
            variationPathNodeIds:
                (h['variationPathNodeIds'] as List<dynamic>?)
                    ?.map((e) => e as int)
                    .toList() ??
                [],
            customVariationValues:
                (h['customVariationValues'] as Map<String, dynamic>?)?.map(
