...
class _SidebarTileState extends State<_SidebarTile> {
  ...
  Widget build(BuildContext context) {
    ...
    final foreground = isSelected
        ? Colors.white
        : (_isHovered ? SoftErpTheme.accentDark : SoftErpTheme.textPrimary);
    final shadow = isSelected ? SoftErpTheme.subtleShadow : const <BoxShadow>[];

    return Material(
      color: Colors.transparent,
      child: InkWell(
        key: ValueKey<String>('sidebar_tile_${widget.item.key}'),
        focusNode: widget.focusNode,
        canRequestFocus: true,
        borderRadius: BorderRadius.circular(widget.compact ? 16 : 30),
        onTap: () {
